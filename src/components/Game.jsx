...
function Game({
  ...
  const handleConfirmPoints = () => {
    ...
    setLastDeltas(
      summarizeDeltas(deltas, generateRoles(roomCode, roundNumber, numPlayers)),
    );
  ...
  const sortedScores = Object.entries(scoreboard).sort(([, a], [, b]) => b - a);
  ...
  return (
    <div className="game">
      ...
      {phase === PHASES.WAZIR_GUESS && (
        <div className="game-phase">
          <h2 className="guess-title">WAZEER, WHO IS THE CHOR?</h2>
          ...
          {showWazirGuessConfirm && (
            <div className="modal-overlay" onClick={cancelWazirGuess}>
              <div className="modal" onClick={(e) => e.stopPropagation()}>
                ...
                <p>
                  This is recorded locally for your reference during the group
                  reveal.
                </p>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="deltas-summary">
              ...
              {getDeltasSummary().map(
                ({ playerNumber: pNum, role: pRole, delta }) => (
                  <div
                    key={pNum}
                    className={`delta-item ${pNum === playerNumber ? "delta-item-you" : ""}`}
                  >
                    <span>
                      Player {pNum} ({pRole})
                      {pNum === playerNumber ? " — You" : ""}
                    </span>
                    <span
            ...
            <p className="sync-warning">
              Points applied locally on this phone. Others must also apply to
            ...
            {lastDeltas && (
              <div className="score-deltas">
                {lastDeltas.map(({ playerNumber: pNum, delta }) => (
                  <div
                    key={pNum}
                    className={`score-delta ${pNum === playerNumber ? "score-delta-you" : ""} ${delta > 0 ? "score-delta-pos" : delta < 0 ? "score-delta-neg" : "score-delta-zero"}`}
                  >
                    <span>
                      Player {pNum}
                      {pNum === playerNumber ? " (You)" : ""}
                  ...
                  {sortedScores.map(([playerNum, score], index) => (
                    ...
                    >
                      ...
                      <div className="scoreboard-name">
                        Player {playerNum}
                        {parseInt(playerNum) === playerNumber && (
          ...
          <div className="game-scores-list">
            {sortedScores.length === 0 ? (
              ...
              sortedScores.map(([playerNum, score]) => (
                <div
                  key={playerNum}
                  className={`game-score-row ${parseInt(playerNum) === playerNumber ? "game-score-you" : ""}`}
                >
                  <span>
                    Player {playerNum}
                    {parseInt(playerNum) === playerNumber ? " (You)" : ""}
                  </span>
